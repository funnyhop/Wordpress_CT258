...
class hacklog_ria_util {
	...
	static function get_link_images($content) {
		$content = preg_replace_callback ( "/<a[^>]*?href=('|\"|)?([^'\"]+)(\\1)[^>]*?>\s*<img[^>]*?src=('|\"|)?([^'\"]+)(\\1)[^>]*?>\s*<\/a>/is", 'hacklog_ria_util::link_img_tag_callback', $content );
		return $content;
	}
	...
	static function get_image_size($img_data) 
	{
		$size = array();
		$img_res = imagecreatefromstring ( $img_data );
		$width = imagesx ( $img_res );
	 ...
	 */
	 ...
	 //Sửa lại để lấy link hình có chứ tiêu đề
	public static function get_filename_from_url($post_id, $remote_image_url, $mime_file_ext)
	{
		$filename = "";
		$posttitle = $remote_image_url;
		...
		$filename = time() .'-'. $random_num .'-'. $filename;
		
		}
		else{
        $postname = sanitize_title($posttitle);
		
		$filename = "$postname-$post_id-$random_num.$mime_file_ext";
	...
	public static function down_remote_file($post_id, $url) {
		...
		if (in_array ( $file_ext, $allowed_filetype )) {
			...
			$image_size = self::get_image_size($file_content);
			//Sửa chỗ này lấy link từ post title
			$filename = self::get_filename_from_url($post_id, $remote_image_url, $file_ext);
			$post = get_post($post_id);
			$posttitle = $post->post_title;
			$type = $mime;
			...
			if (! is_wp_error ( $result ['id'] )) {
				...
				$img = wp_get_attachment_image_src ( $result ['id'], $size);
				$img_width = $image_size[0]['width'];
				$img_height = $image_size[0]['height'];
				$full_image = wp_get_attachment_image_src ( $result ['id'], $size );
				
				$link_class =  hacklog_remote_image_autosave::get_conf ( 'css_class_link', '' );
				$img_class =  hacklog_remote_image_autosave::get_conf ( 'css_class_img', '' );
				$insert_link = hacklog_remote_image_autosave::get_conf('link_image',"1");
				$html = '';
				if($insert_link=="1"){
				$html .= '<a class="'. $link_class .'" title="'. esc_html($posttitle) .'" href="' . $full_image [0] . '">';
				}
				$html .= '<img class="'. $img_class .'" alt="'. esc_html($posttitle) .'" src="' . $img[0] . '"/>';
				if($insert_link=="1")
				{
				$html .= '</a>';
				}
